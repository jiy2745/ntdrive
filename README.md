...
| `vm_create` | additive | Create a new, unencrypted VM (empty 64 GB disk, UEFI, e1000e NIC for KDNET) and register it, optionally booting an installer ISO. This is how to get a guest that vmrun CAN clone: no encryption and no vTPM, unlike an encrypted base. Install the OS into it once, then vm_clone it freely. |
| `vm_register` | additive | Register an existing VM (a manual or GUI clone) as a new ntdrive VM, inheriting the template VM's guest, debugger and encryption config with its own fresh KDNET port. For encrypted VMs, which no CLI can clone: clone once in the VMware GUI, then register the copy here. |
...
| `kd_bugcheck` | read | Classify the current bugcheck cheaply: runs .bugcheck (two lines) and returns the code, its arguments and the faulting instruction. Use this before !analyze -v, which takes tens of seconds and prints tens of kilobytes of chkimg noise that is false on a patched kernel. |
