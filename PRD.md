...
| `vm_create` | `vm (template), name, iso?, cpus?, memory_mb?, guest_os=windows11-64` | `{vm, template, vmx, disk, guest_os, encrypted: false, iso, hardware?, kd_transport, kdnet_port?, next}`. Creates a new VM with `vmcli VM Create` (vmrun cannot create one) and wires up what vmcli leaves out: the disk, UEFI firmware, an e1000e NIC for KDNET and the installer ISO. It inherits the template's guest and debugger config but deliberately **not** its encryption, so `vm_clone` works on the result. Install the OS once, then clone freely |
...
| `kd_sample` | `vm, symbol, exprs[], n=8, condition?, max_seconds=120, timeout=60` | `{hits, rows:[{hit, values:{expr: output}, bugcheck?}], skipped_by_condition, stopped_because: n\|max_seconds\|timeout\|bugcheck, elapsed_s, state}`. Breaks on `symbol`, resumes with a plain `g`, evaluates `exprs` at each hit and clears only the breakpoint it set. `condition` is evaluated by the daemon with `?` and never compiled into the breakpoint, because a conditional breakpoint with `gc` on a hot function NMIs the guest. Both `n` and `max_seconds` bound it |
